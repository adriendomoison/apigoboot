...
func (s *service) CheckCredentials(username string, password string, authType string) (userId uint, ok bool) {
	if authType == "password" {
		return s.CheckCredentialsForPasswordAuth(username, password)
	} else if authType == "facebook" {
		return s.CheckCredentialsForFacebookAuth(username, password)
	} else if authType == "google" {
		return s.CheckCredentialsForGoogleAuth(username, password)
	}
	return 0, false
}

// CheckCredentialsForPasswordAuth check user credentials in database
func (s *service) CheckCredentialsForPasswordAuth(email string, password string) (userId uint, ok bool) {
...
func (s *service) CheckCredentialsForFacebookAuth(facebookUserId string, accessToken string) (userId uint, ok bool) {
...
func (s *service) CheckCredentialsForGoogleAuth(googleUserId string, accessToken string) (userId uint, ok bool) {
	conf := &oauth2.Config{
